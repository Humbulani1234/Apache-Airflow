...
sql_data = "INSERT INTO securities(Symbol, Security, SEC_filings, GICS_Sector, GICS_Sub_Industry, Headquaters_Location) VALUES (%s,%s,%s,%s,%s,%s)"
...
#a = new.Get_sql_table(header_data, table_name)
#print(a)
...
with DAG (dagrun_timeout=timedelta(minutes=60),dag_id="mysql_dag", start_date=datetime(2023,3,15),
    ...
    #json_data_2 = json.dumps({'sql_data':'INSERT INTO securities (Symbol, Security, SEC_filings, GICS_Sector, 
                                                                   # GICS_Sub_Industry, Headquaters_Location) VALUES (%s,%s,%s,%s,%s,%s)'},indent=1)
    
    start_pipeline = EmptyOperator(task_id="start_pipeline")
    
    create_table = MySqlOperator(task_id="mysql_connection", sql=sql_create, mysql_conn_id="mysql_connection")

    task1 = PythonOperator(task_id="insert_data", python_callable=INSERT_INTO_TABLE, op_kwargs=Variable.get("mysql_dag_settings", deserialize_json=True))
